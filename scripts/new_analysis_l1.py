...
parser.add_argument("--plot-abs", help="Plot absolute MPKI instead of normalized MPKI", action="store_true")
...
expt_list = ["lru_64", "lru_8", "srrip_64", "hru_64", "hru_8"]
...
expt_order = ["lru_64", "lru_8", "srrip_64", "hru_64", "hru_8"]
baseline_key = "lru_64"
...
for cache_type in ["L1D"]:
    ...
    for i, expt in enumerate(expt_order):
        ...
        # 4. Plot the bars
        if (args.plot_abs):
         bars = ax.bar(indices + offset, current_values, width=bar_width, 
    ...
    ax.set_title(f"Normalized MPKI with Aggregate Geomean for {args.graph_type}-ordered Graphs\n{cache_type} - {cache_size}MB Cache", fontsize=14)
