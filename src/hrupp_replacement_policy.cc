...
void HRUpp::breakdown(std::vector<VecType>& vec, uint64_t prev_num_ways, uint64_t scale_factor, bool /*incr*/) {
...
void HRUpp::set_breakdown(uint64_t old_block_size, uint64_t new_block_size) {
...
template<typename VecType>
void HRUpp::contract(std::vector<VecType>& vec, uint64_t way_idx, uint64_t num_blocks) {
...
void HRUpp::set_contract(uint64_t way_idx, uint64_t size) {
    // First, resize all the vectors to break down their contents
