...
void HRRIP::fill_update(uint64_t way_idx, uint64_t /*block_idx*/, PacketPtr packet, bool /*was_accessed*/) {
    if (packet->serviced_from_llc > 0) {
        ...
        counter[way_idx] = packet->is_hub_node ? 0 : maxRRPV - 1;
    }

    if (cachesim::DEBUG || cachesim::REPLACEMENT_POLICY_DEBUG)
        fmt::print("Fill Update. PC {:#x} address {:#x} way {} hub {} degree {} l1_hits {} repl_policy_new {}\n", packet->pc, packet->address, way_idx, packet->is_hub_node,
...
void HRRIP::breakdown(std::vector<VecType>& vec, uint64_t prev_num_ways, uint64_t scale_factor, bool /*incr*/) {
...
void HRRIP::set_breakdown(uint64_t old_block_size, uint64_t new_block_size) {
...
template<typename VecType>
void HRRIP::contract(std::vector<VecType>& vec, uint64_t way_idx, uint64_t num_blocks) {
...
void HRRIP::set_contract(uint64_t way_idx, uint64_t size) {
    contract(counter, way_idx, size);
    num_ways -= size;
}

void HRRIP::set_merge(uint64_t /*old_block_size*/, uint64_t /*new_block_size*/, uint64_t new_num_ways) {
